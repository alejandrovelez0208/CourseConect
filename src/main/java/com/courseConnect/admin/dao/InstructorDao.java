...
public interface InstructorDao extends JpaRepository<Instructor, Long> {
	...
	@Query(value = "select i.* from Instructores as i where i.nombres like %:name% or i.apellidos like %:name%", nativeQuery = true)
	List<Instructor> findInstructoresByNombre(@Param("name") String name);

	@Query(value = "select i.* from Instructores as i where i.usuario.email =:email", nativeQuery = true)
